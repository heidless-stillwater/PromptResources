...
async function debug() {
    console.log('--- Checking for Attribution Pollution ---');
    ...
    resources.docs.forEach(doc => {
        ...
        const uids = data.attributedUserIds || [];
        
        const hasDA = names.includes('Digital Assets');
        const hasRob = names.some(n => n.toLowerCase() === 'rob' || n.toLowerCase() === 'heidless');
        
        if (hasDA && hasRob) {
            console.log(`Polluted Resource: ${doc.id} ("${data.title}")`);
    ...
    console.log(`\nFound ${count} polluted resources.`);
}
