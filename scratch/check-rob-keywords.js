...
async function debug() {
    const snap = await db.collection('resources').where('attributedUserIds', 'array-contains', 'nNdenyyfKaN9yNB9Ly3vhhaHLXx1').limit(1).get();
    if (!snap.empty) {
        console.log(JSON.stringify(snap.docs[0].data().searchKeywords, null, 2));
    }
}
