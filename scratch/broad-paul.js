...
async function debug() {
    console.log('--- Broad Paul Search ---');
    const snap = await db.collection('resources').get();
    snap.docs.forEach(doc => {
        const data = doc.data();
        const names = (data.attributionNames || []).join(', ').toLowerCase();
        if (names.includes('paul') || names.includes('lipsk')) {
            console.log(`Match: ${doc.id} ("${data.title}") -> Names: ${JSON.stringify(data.attributionNames)}`);
        }
    });
}
